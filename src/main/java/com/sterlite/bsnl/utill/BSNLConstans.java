...
public class BSNLConstans {
	...
	public static final String sqlSingleBNGInvStatus = "select B.STAGENAME,A.STATUS,A.CLOSEDATE,A.TARGETDATE, A.JV,B.ORDERID,A.bngid,A.REMARKS  from TBLBNGINVOICEANFAPYMENTSTAGE A, TBLBNGINVOICEANDPAYMENTSTAGEODER B where B.STAGEID=A.STAGEID and A.bngid= :bngid order by B.STAGEID";
	...
	public static final String sqlZoneWiseSummary = "select d.phase,d.zonename,count(d.bngid) as 'Total BNG',count(if(d.SiteSurvey='Closed','Closed',NULL)) as 'SiteSurvey',count(if(d.SiteReady='Closed','Closed',NULL)) as 'Site Ready',\r\n" + 
			"count(if(d.MaterialDelivery='Closed','Closed',NULL)) as 'Material Delivery',count(if(d.PowerOn='Closed','Closed',NULL)) as 'Power On',count(if(d.NWIntegration='Closed','Closed',NULL)) as 'NW Integration',count(if(d.AT='Closed','Closed',NULL)) as 'AT',count(if(d.Commissinong='Closed','Closed',NULL)) as 'Commissioning',count(if(d.ATC='Closed','Closed',NULL)) as 'ATC',\r\n" + 
			"count(if(d.ERPPO='Closed','Closed',NULL)) as 'ERP PO',count(if(d.MIGO='Closed','Closed',NULL)) as 'MIGO',count(if(d.MIRO='Closed','Closed',NULL)) as 'MIRO',count(if(d.PaymentStatus='Closed','Closed',NULL)) as 'Payment Status'\r\n" + 
			"from\r\n" + 
			"(\r\n" + 
	...
	public static final String sqlCircleWiseSummary = "select d.phase,d.circlename,d.circlecode,count(d.bngid) as 'Total BNG',count(if(d.SiteSurvey='Closed','Closed',NULL)) as 'SiteSurvey',count(if(d.SiteReady='Closed','Closed',NULL)) as 'Site Ready',\r\n" + 
			"count(if(d.MaterialDelivery='Closed','Closed',NULL)) as 'Material Delivery',count(if(d.PowerOn='Closed','Closed',NULL)) as 'Power On',count(if(d.NWIntegration='Closed','Closed',NULL)) as 'NW Integration',count(if(d.AT='Closed','Closed',NULL)) as 'AT',count(if(d.Commissinong='Closed','Closed',NULL)) as 'Commissioning',count(if(d.ATC='Closed','Closed',NULL)) as 'ATC',\r\n" + 
			"count(if(d.ERPPO='Closed','Closed',NULL)) as 'ERP PO',count(if(d.MIGO='Closed','Closed',NULL)) as 'MIGO',count(if(d.MIRO='Closed','Closed',NULL)) as 'MIRO',count(if(d.PaymentStatus='Closed','Closed',NULL)) as 'Payment Status'\r\n" + 
			"from (\r\n" + 
	...
	public static final String updateBNGInsStageDetail= "update TBLBNGINSTANDCOMMISTAGE A set A.TARGETDATE =:TDATE, A.CLOSEDATE=:CLDATE, A.STATUS=:STATUS ,A.REMARKS=:REMARK,A.LASTMODIFIEDDATE=SYSDATE(), A.LATMODIFIEDBY=1 where BNGID=:bngId and  A.STAGEID=:stageId";
	...
	public static final String updateBNGInvStageDetail= "update TBLBNGINVOICEANFAPYMENTSTAGE A set A.TARGETDATE =:TDATE, A.CLOSEDATE=:CLDATE, A.STATUS=:STATUS ,A.REMARKS=:REMARK,A.LASTMODIFIEDDATE=SYSDATE(), A.LATMODIFIEDBY=1 where BNGID=:bngId and  A.STAGEID=:stageId";
